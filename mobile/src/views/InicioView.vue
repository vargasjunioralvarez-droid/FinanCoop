  ...
  <v-container class="pa-4 inicio-view">
    ...
    <v-slide-group show-arrows class="mb-4">
      <v-slide-group-item
        ...
        :key="fin.codigo"
      >
        <v-card class="compra-card mx-2" width="280" elevation="4">
          <v-card-text class="pa-4">
            <div class="d-flex align-center mb-3">
            ...
            <v-btn
              v-if="fin.proxima_cuota && fin.proxima_cuota.puede_pagar"
              color="success"
              block
              size="small"
              class="mt-3"
              rounded="pill"
              @click="$emit('pagar', fin.proxima_cuota)"
            >
    ...
    <div v-if="cuotasProximas.length > 0" class="mb-4">
      ...
      >
        <v-card-text class="pa-3 d-flex align-center">
          ...
          <div class="flex-grow-1">
            <div class="text-body-2 font-weight-medium">
              Cuota {{ cuota.cuota_numero }} — {{ cuota.financiamiento_descripcion }}
            </div>
            <div class="text-caption text-medium-emphasis">
          ...
          <div class="text-right">
            <div class="text-body-1 font-weight-medium" style="font-variant-numeric: tabular-nums;">
              BS {{ formatearBS(cuota.monto_total_bs) }}
            </div>
            <div class="text-caption text-medium-emphasis">
              ${{ formatearUSD(cuota.monto_total_usd_ref) }}
            </div>
          </div>
